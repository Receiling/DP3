...
class ARMTPP(nn.Module):
    def __init__(self, cfg, args):
        ...
        self.event_linear = nn.Linear(self.cfg.RNN_HIDDEN_DIM, 1)
    ...
    def forward(self, input, length):
        ...
        attn_weight = torch.abs(self.tanh(self.attn(output)))
        ...
        event_output = self.event_linear(attn_applied).squeeze()
        event_output = self.softmax(event_output)
        selected_event = torch.argmax(event_output, dim=1)
        event_output = torch.log(event_output)
        selected_attn_applied = torch.cat([attn_applied[i, selected_event[i], :].unsqueeze(0)
