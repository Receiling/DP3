...
def generate_time_sequence(csv_file,
    ...
    with open(time_sequences_file, 'w') as time_sequences_fout:
                ...
                if time_sequence is not None and min_length <= len(time_sequence):
                    for ed in range(min_length - 1, len(time_sequence)):
                        ...
                        time_sequences_fout.write(','.join(
                            time_sequence[max(ed - min_length + 1, ed - max_length + 1):ed]) + '\n')
                last_id = id
        ...
        if time_sequence is not None and min_length <= len(time_sequence):
            for ed in range(min_length - 1, len(time_sequence)):
                time_sequences.append(time_sequence[max(ed - min_length + 1, ed - max_length +
                                                        1):ed])
                time_sequences_fout.write(','.join(time_sequence[max(ed - min_length + 1, ed -
                                                                     max_length + 1):ed]) + '\n')
...
def generate_event_sequence(csv_file,
    ...
    with open(event_sequences_file, 'w') as event_sequences_fout:
                ...
                if event_sequence is not None and min_length <= len(event_sequence):
                    for ed in range(min_length - 1, len(event_sequence)):
                        ...
                        event_sequences_fout.write(','.join(
                            list(
                                map(
                                    str, event_sequence[max(ed - min_length + 1, ed - max_length +
                                                            1):ed]))) + '\n')
                last_id = id
        ...
        if event_sequence is not None and min_length <= len(event_sequence):
            for ed in range(min_length - 1, len(event_sequence)):
                event2index[event_sequence[ed]]['cnt'] += 1
                event_sequences.append(event_sequence[max(ed - min_length + 1, ed - max_length +
                                                          1):ed])
                event_sequences_fout.write(','.join(
                    list(map(str, event_sequence[max(ed - min_length + 1, ed - max_length +
                                                     1):ed]))) + '\n')
...
def generate_sequence(csv_file,
        ...
        if last_id is None or last_id != id:
            if event_sequence is not None and min_length <= len(event_sequence):
                for ed in range(min_length - 1, len(event_sequence)):
                    ...
                    event_sequences_fout.write(','.join(
                        list(
                            map(str, event_sequence[max(ed - min_length + 1, ed - max_length +
                                                        1):ed]))) + '\n')
            last_id = id
            time_sequence = [timestamp]
            event_sequence = [event]
    ...
    if event_sequence is not None and min_length <= len(event_sequence):
        for ed in range(min_length - 1, len(event_sequence)):
            ...
            event_sequences_fout.write(','.join(
                list(map(str, event_sequence[max(ed - min_length + 1, ed - max_length + 1):ed]))) +
                                       '\n')
...
if __name__ == '__main__':
    ...
    generate_event_sequence('../data/real/atm/atm_day.csv',
                            ...
                            train_rate=0.8)
    sequences = load_sequences('../data/real/atm/time.txt', '../data/real/atm/event.txt')
    print(sequences[0])
    print(len(sequences))
